<div x-data="{ show: @entangle('abierto') }" x-show="show" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" style="display: none;">
    <div class="bg-white dark:bg-gray-900 w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-gray-800">
        ...
        <div class="p-6 md:p-8">
            <form wire:submit.prevent="actualizar" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <x-input-form id="precio" label="Precio por Unidad ($)" model="precio" type="number" step="0.01" simbolo="$" />
                    <x-input-form id="stock" label="Stock por Día (Cantidad disponible)" model="stock" type="number" />
                </div>

                <x-textarea-form id="descripcion" label="Descripción y Condiciones" model="descripcion" rows="4" />
