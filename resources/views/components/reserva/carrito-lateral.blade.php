    ...
    <div class="bg-gray-900 rounded-[2.5rem] p-6 md:p-8 shadow-2xl sticky top-6 text-white border border-white/10 overflow-hidden">
        ...
        @else
                    ...
                    <div class="bg-white/5 p-3 md:p-4 rounded-2xl border border-white/5 hover:border-white/20 transition-all relative group" wire:key="cart-item-{{ $index }}">
                        @if($paso === 1)
                            <button wire:click="$dispatch('quitar-del-carrito', { index: {{ $index }} })"
                                class="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-all shadow-xl z-10">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        @endif
                        <p class="font-black text-[10px] md:text-[11px] leading-tight mb-2 uppercase text-gray-200 truncate pr-4">{{ $item['nombre'] }}</p>
                        <div class="flex justify-between items-center gap-2">
                            <div class="flex gap-1.5">
                                <span class="bg-white/10 px-2 py-0.5 rounded text-[8px] md:text-[9px] font-black uppercase shrink-0">
                                    {{ $item['cantidad'] }} Un.
                                </span>
                                @if($aplicaDias)
                                    <span class="bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded text-[8px] md:text-[9px] font-black uppercase shrink-0">{{ $dias }} Días</span>
                                @endif
                                @if($esHospedaje && ($item['capacidad_aportada'] ?? 0) > 0)
                                    <span class="bg-orange-500/20 text-orange-300 px-2 py-0.5 rounded text-[8px] md:text-[9px] font-black uppercase shrink-0">
                                        {{ $item['capacidad_aportada'] }} Pers.
                                    </span>
                                @endif
                            </div>
            ...
            <div class="pt-6 border-t border-white/10 space-y-6">
                ...
                @if($paso === 1)
                    <button wire:click="$set('paso', 2)" class="w-full py-4 md:py-5 bg-green-500 hover:bg-green-400 text-gray-900 font-black rounded-2xl transition-all shadow-lg uppercase text-[10px] md:text-xs tracking-widest">
                        Siguiente Paso
                    </button>
                @endif
